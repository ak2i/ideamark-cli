...
doc_id: "ideamark.cli.v0.1.3.dev-spec.v0.0.23.2026-03-04"
doc_type: "evolving"
status:
  state: "in_progress"
...
# IdeaMark-CLI v0.1.3 Development Spec (v0.0.23)

v0.0.6 / v0.0.17 / v0.0.20 を統合再構成した網羅版。  
構造は v0.0.20、内容は v0.0.17 を吸収し、v0.0.6 監査済み。

v0.0.23 では、v0.1.3 開発を止めない方針で **diff/lint（診断ツール）** を追加し、SEC-IMPLEMENTATION-PLAN に diff/lint 実装手順を組み込む。
...
- `rule_id` : 例 `IM-LINT-001`
- `severity` : `info|warning|error`
- `path` : ファイルパス（可能なら）
- `location` : `header|section|occurrence|entity|relation|structure` 等
- `pointer` : `#/entities/IE-...` 等（可能なら）
- `message` : 人間向け短文
- `hint` : 次に取るべき行動（追加材料/局所修正/再実行コマンド）
- `evidence` : 機械向けの補助（任意）
