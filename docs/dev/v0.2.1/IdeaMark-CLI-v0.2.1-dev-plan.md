# IdeaMark-CLI v0.2.1 Development Plan
...
v0.2.1 の目的は、IdeaMark Core **v1.2.0(Part 4 Core Specification)** に CLI を追従させることです。
...
したがって v0.2.1 は v0.2.0 のときと同様に「コアモデルの前提差し替え」であり、パーサ・内部モデル・バリデータを新規に作り直すことを主眼とします。

なお V1.2.0 仕様は **Status: Draft** です。仕様側 README も「今後の焦点は CLI と authoring tool の実装計画」と明言しており、CLI 実装は仕様レビュー(Part 4 `19-open-review-issues.md`)へのフィードバックループを兼ねます。実装中に発見した仕様の曖昧点は `docs/dev/v0.2.1/spec-feedback.md` に記録します。
...
### Out of Scope(v0.2.2 以降へ後送り)

1. `extract` / `compose` / `publish` / `diff` の v1.2.0 対応(新モデルでの意味論を再設計してから実装する。v0.2.1 では v1.2.0 入力に対し明示的な `unsupported` エラーを返す)
...
5. `lint` の v1.2.0 プロファイル再設計(validate の warning 群で v0.2.1 は代替。lint 独自ルールは v0.2.2 で再検討)
...
Part 4 は診断コードを CLI に委譲しています(§19.7.1)。v0.2.1 で `docs/dev/v0.2.1/diagnostic-codes.md` をレジストリとして起こし、コード → Part 4 章番号の対応を明記します。命名は現行の snake_case を踏襲します。
...
- コマンド仕様(フラグ・exit code・入出力)を `cli-contract-v1.2.0.md` としてドラフト
  - `validate [--mode core|strict] [--fail-on-warn]`(既定 core)
  - `format [--canonical]`
...
- README / guides / `package.json`(`files` に `docs/dev/v0.2.1/` を追加)更新
...
7. **Smoke**: `tests/run-smoke-v0.2.1.js` として CLI E2E(validate / format / migrate / ls / describe)
...
- D1: v0.2.1 は v1.2.0 を唯一の正規仕様とし、v1.1.1 は `migrate` 入力としてのみ扱う(v0.2.0 方針 A の踏襲)
- D2: `extract/compose/publish/diff/lint` の v1.2.0 再設計は v0.2.2 以降(スコープ肥大の回避)
- D3: validate の既定モードは Core mode。旧 `working` モード名は廃止
- D4: 診断コードは CLI レジストリで管理し、Part 4 章番号と対応付ける

### オープン課題(実装中に判断・仕様側へフィードバック)

- O1: placeholder 判定の厳密な条件(status: draft を要件にするか、単なるフィールド欠落で足りるか)— Part 4 §3.14 は例示のみ
- O2: `meta.projections.inline` のサイズ・スコープ警告をどこまで出すか(§19.4.2 が未解決)
- O3: 未知 `spec_version` の扱い — SHOULD reject を error にするか、`--allow-unsupported-spec` のような逃げ道を設けるか
- O4: strict mode の reject 対象のうち MAY 項目(コメント付き YAML 等)をどこまで既定で有効にするか
- O5: サンプルが使う anchor `role`(例: `procedure_window`)は §19.4.3 でまだ example 掲載が未決 — CLI は open vocabulary として warn なしで通す想定
- O6: v0.2.1 という patch 番号でモデル破壊的変更を出すことの是非(npm 配布物としては 0.x なので semver 上は許容だが、v0.3.0 への昇格も検討余地あり)
...
- [ ] README / guides 更新、`package.json` version = 0.2.1
