# IdeaMark WorkCell: Decision6 WorkCell（ideamark CLI v0.1.1準備 / 開発版 v0.1）
...
## SEC-V0-1-1-WORKCELL-NOTES

```yaml
section_id: "SEC-V0-1-1-WORKCELL-NOTES"
anchorage:
  view: "design"
  phase: "implementation"
```
...
- `ideamark ls` を追加し、extract/compose/publish の前に対象文書のID・語彙を素早く把握できるようにする。
- `ideamark describe ai-authoring/params` を追加し、responder-bridge が LLMプロンプトに組み込める
  “固定の部品” を提供する。
- `--spec`/`--pattern` による spec 抽出は将来の拡張として残し、v0.1.1では固定テンプレで運用する。
