# IdeaMark CLI v0.3.0 — Validation Rule Checklist

**Target spec:** IdeaMark Core v1.2.0, Part 4 (`docs/specs/V1.2.0/part4-core-specification/`)  
**Status:** Phase 0 draft (frozen for v0.3.0 implementation unless a spec change forces revision)
...
| NS-05 | §2.7–2.8, §12.9 | MAY | `relations` / `perspectives` / `provenance` content is preserved, never validated in v0.3.0 | — | — | — |
...
| META-06 | §4.7 | SHOULD | Declared `meta.profiles` supported by this tool. v0.3.0 supports no profiles → any declaration warns "profile validation not performed" | warn | error | `profile_unsupported` |
...
| OCC-07 | §8.6–8.7, §9.8 | [dev] | `role` value vocabulary is fully open in Core mode: no diagnostic for unlisted roles (no profile support in v0.3.0). Resolves the §8.6 vs §13.12 tension in favor of §13.12 — see F3 | — | — | — |
...
| ENT-08 | §9.8–9.9 | [dev] | `kind` vocabulary is fully open in Core mode (§9.8: warn only when a profile declares a vocabulary; no profile support in v0.3.0) | — | — | — |
...
| EXT-02 | §3.10, §12.5, §13.11 | MAY | Clearly namespaced extension fields (`x-*`) and `extensions` namespace content: preserved; no Core diagnostic. Strict: rejected as undeclared (no profile support in v0.3.0, D8) | info | error | `undeclared_extension` |
