# IdeaMark CLI v0.3.1 — Diagnostic Code Registry
...
| Code | Severity | Meaning |
| --- | --- | --- |
| `unsupported_for_spec` | error | Command does not support this document's spec version in v0.3.1 (`extract` / `compose` / `publish` / `diff` / `lint` on v1.2.0 input) |

## Skeleton Graph diagnostics (v0.3.1)
...
Core mode emits these diagnostics as warnings because `skeletons` is optional Core-adjacent content.
